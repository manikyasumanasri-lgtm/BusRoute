...
def register():
    if request.method == "POST":
        ...
        if username in users: flash("Username already exists.", "error"); return render_template("register.html",
                                                                                                 logged_in=False)
        users[username] = {"pw": generate_password_hash(password), "email": email}
...
def simulate(bus_id):
    ...
    flash("Simulated one tracking update.", "success")
...
if __name__ == "__main__":
    ...
    logging.info("2. Login and simulate GPS or generate test data")
